;;; jde-ecj-flymake.el --- JDEE flymake support when using Eclipse compiler as a compile server.
...
;; compiler is selected as a compile server to `jde-compiler'.
;;
;; Usage:
;;
...
(defcustom jde-ecj-command-line-args '("-d" "none" "-source" "1.6" "-target" "1.6" "-warn:-serial")
...
To use this funtion set the java line in `flymake-allowed-file-name-masks' to
  ...
  (if (not (object-of-class-p (jde-compile-get-the-compiler) 'jde-compile-ejc-server))
    ...
    (let* ((temp-file (flymake-init-create-temp-buffer-copy 'jde-ecj-create-temp-file))
           ...
           (bsh-process (oref (oref  (jde-ecj-get-bsh) buffer) process)))
...
(defun jde-ecj-reset-bsh ()
  "Reset things when ecj bsh gets wedged"
  (interactive)
  (setq jde-ecj-server-setup-called nil)
  (jde-ecj-bsh-exit))

(defun jde-ecj-bsh-exit ()
  "Closes the existing ecj beanshell process"
  (interactive)
  (if (jde-ecj-bsh-running-p)
      (let ((process (bsh-get-process (jde-ecj-get-bsh))))
